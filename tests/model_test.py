...
@pytest.mark.parametrize("name, version, num_files", [("onnx_and_config", 1, 2), ("only_onnx", 1, 1), ("onnx_and_config", 2, 2)])
...
@pytest.mark.parametrize("name, version", [("wrong_model_name", 1),
                                                      ("no_model", 1),])
...
def test_create_model_unsuccess_already_existing(test_client, test_storage, test_settings, create_zips, name, version, num_files):
    path_to_zip = test_settings.zips_path / (name + ".zip")
    with open(path_to_zip, "rb") as package:
        response = test_client.post(
            "/models",
...
@pytest.mark.order(after="test_create_model_success")
@pytest.mark.parametrize("name, version, expected_json", [("onnx_and_config", None, [{"name": "onnx_and_config", "version": 1}, {"name": "onnx_and_config", "version": 2}]), ("only_onnx", 1, [{"name": "only_onnx", "version": 1}]),
                                                          (None, None, [{"name": "onnx_and_config", "version": 1}, {"name": "only_onnx", "version": 1}, {"name": "onnx_and_config", "version": 2}]),
                                                          ("not_existent", None, []), (None, 1, [{"name": "onnx_and_config", "version": 1}, {"name": "only_onnx", "version": 1}]),
                                                          ])
...
@pytest.mark.order(after="test_create_model_success")
@pytest.mark.parametrize("name, version, expected_code", [("onnx_and_config", 3, 404),
                                                     ("not_existent", 1, 404),
